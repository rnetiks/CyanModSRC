...
public class Emitter
{
    ...
    public Vector3 GetEmitRotation(EffectNode node)
    {
        ...
        if (this.Layer.EmitType == 3)
        {
            ...
            Vector3 toDirection = Vector3.RotateTowards(vector, this.Layer.CircleDir, (90 - this.Layer.AngleAroundAxis) * 0.01745329f, 1f);
            return (Vector3) (Quaternion.FromToRotation(vector, toDirection) * vector);
        }
        if (this.Layer.IsRandomDir)
        {
            Quaternion quaternion = Quaternion.Euler(0f, 0f, (float) this.Layer.AngleAroundAxis);
            Quaternion quaternion2 = Quaternion.Euler(0f, (float) UnityEngine.Random.Range(0, 360), 0f);
            return (Vector3) (((Quaternion.FromToRotation(Vector3.up, this.Layer.OriVelocityAxis) * quaternion2) * quaternion) * Vector3.up);
        }
