...
public class InstantiateTracker
{
    ...
    {
        if (photonPlayer != null && key != null && viewIDS != null && key.Trim() != "" && viewIDS.Length != 0)
        {
            ...
            switch (key)
            {
                ...
                case "fx/rockthrow":
                case "fx/bite":
                    if ((!FengGameManagerMKII.lvlInfo.teamTitan && (RCSettings.infectionMode <= 0)) && (IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.BOSS_FIGHT_CT))
                    {
                ...
                case "titan_eren":
                    if (photonPlayer.character.ToUpper() != "EREN")
                    {
                        if (PhotonNetwork.isMasterClient)
                        {
                ...
                case "horse":
                    if (!FengGameManagerMKII.lvlInfo.horse && (RCSettings.horseMode == 0))
                    {
                ...
                case "titan_ver3.1":
                    ...
                    if (PhotonNetwork.isMasterClient)
                    {
                        ...
                        {
                            FengGameManagerMKII.instance.kickPlayerRC(photonPlayer, false, CyanMod.INC.la("kiked_player_titan"));
                            return false;
                        }
                        if ((FengGameManagerMKII.lvlInfo.teamTitan || (RCSettings.infectionMode > 0)) || ((IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.BOSS_FIGHT_CT) || FengGameManagerMKII.instance.restartingTitan))
                        {
                ...
                case "titan_new_2":
                    ...
                    return (!FengGameManagerMKII.masterRC && this.Instantiated(photonPlayer, GameResource.general));
    ...
    public bool Instantiated(PhotonPlayer owner, GameResource type)
    {
        ...
        if (this.TryGetPlayer(owner.ID, out num))
        {
            if (this.players[num].IsThingExcessive(type))
            {
                ...
                cext.RemoveAt<Player>(ref this.players, num);
                return false;
            }
        }
        else
        {
            cext.Add<Player>(ref this.players, new Player(owner.ID));
    ...
    public void TryRemovePlayer(int playerId)
    {
        for (int i = 0; i < this.players.Length; i++)
        {
            ...
            {
                cext.RemoveAt<Player>(ref this.players, i);
                return;
            }
        }
    }
    ...
    private class Player
    {
        ...
        public bool IsThingExcessive(InstantiateTracker.GameResource gr)
        {
            ...
            cext.Add<InstantiateTracker.ThingToCheck>(ref this.thingsToCheck, this.GameResourceToThing(gr));
            return false;
        }
