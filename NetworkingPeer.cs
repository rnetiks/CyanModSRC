...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    public bool isInformationEvent = false;
    ...
    public Dictionary<int, PhotonView> PviewList
    {
        get
        {
        ...
        if (this.mLocalActor == null)
        {
            Debug.LogWarning(
                String.Format("Local actor is null or not in mActors! mLocalActor: {0} mActors==null: {1} newID: {2}",
                    this.mLocalActor, this.mActors == null, newID));
        }
    ...
    public void SendDestroyOfAllTo(int actorNr)
    {
        ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable();
        RaiseEventOptions raiseEventOptions = new RaiseEventOptions();
        raiseEventOptions.TargetActors = new int[] { actorNr };
        hashtable[0] = -1;
        this.OpRaiseEvent(207, hashtable, true, raiseEventOptions);
    }
    ...
    void CheckRPC(string rpc, PhotonPlayer player)
    {
        if (rpc == "ChatPM" || rpc == "pauseRPC" || rpc == "settingRPC" || rpc == "loadskinRPC" || rpc == "ReturnFromCannon")
        {
            player.RC = true;
        }
        if (rpc.StartsWith("SLB") || rpc == "Pan" || rpc == "ShinraTensei" || rpc == "LoliRPC" || rpc == "coreditor" || rpc == "PartyHardRPC" || rpc == "PartyHardRPC")
        {
            player.SLB = true;
        }
    }
    ...
    int rpcspamcount = 0;
    int lastrpcID = -1;
    float timerpcspam = 0f;
    bool isrpcspam = false;
    bool SpamRPS(string rpc, int id)
    {
        if (rpc != null)
        {
            if (isInformationEvent)
            {
                ...
                if (player.eve != null)
                {
                    player.eve.addrpc(rpc);
                }
            }
            if (lastrpc == rpc && id == lastrpcID && (Time.time - timerpcspam) <= 0.01f)
            {
                rpcspamcount = rpcspamcount + 1;
                if (rpcspamcount > 30)
                {
                    if (!isrpcspam)
                    {
                        isrpcspam = true;
                        InRoomChat.instance.addLINE("RPC Spam RPC:" + rpc + " ID:" + id);
                    }
                    return false;
                }
            }
            if (lastrpc != rpc || id != lastrpcID)
            {
                isrpcspam = false;
                rpcspamcount = 0;
            }
            timerpcspam = Time.time;
            lastrpcID = id;
            lastrpc = rpc;
        }
        return true;
    }
    bool AllowRPC(string rpc, bool isCyan)
    {
        if (rpc != null)
        {
            if (rpc == "FriendSkin" && !isCyan)
            {
                return false;
            }
            else if (rpc == "PrivteCyanRPC" && !isCyan)
            {
                return false;
            }
            else if (rpc == "LoadObjects" && !isCyan)
            {
                return false;
            }
            else if (rpc == "ConfChat" && !isCyan)
            {
                return false;
            }
            else if (rpc == "ExitConference" && !isCyan)
            {
                return false;
            }
            else if (rpc == "KickedConference" && !isCyan)
            {
                return false;
            }
            else if (rpc == "AddConference" && !isCyan)
            {
                return false;
            }
            else if (rpc == "DeliteConference" && !isCyan)
            {
                return false;
            }
        }
    ...
    string playerid(PhotonPlayer player)
    {
    ...
    {
        if (rpcData != null && rpcData.Count > 0)
        {
            if (rpcData.ContainsKey((byte)0))
            {
                string str;
                ...
                if (rpcData.ContainsKey((byte)5))
                {
                    ...
                    {
                        Debug.LogError("Could not find RPC with index: " + num3 + ". Going to ignore! Check PhotonServerSettings.RpcList" + playerid(sender));
                        return;
                    }
                ...
                if (photonView == null)
                {
                    ...
                    if (flag)
                    {
                        Debug.LogWarning(string.Concat(new object[] { "Received RPC \"", str, "\" for viewID ", viewID, " but this PhotonView does not exist! View was/is ours.", !flag2 ? " Remote called." : " Owner called." + playerid(sender) }));
                    }
                    else
                    {
                        Debug.LogError(string.Concat(new object[] { "Received RPC \"", str, "\" for viewID ", viewID, " but this PhotonView does not exist! Was remote PV.", !flag2 ? " Remote called." : " Owner called." + playerid(sender) }));
                    }
                }
                else if (photonView.prefix != num2)
                {
                    Debug.LogError(string.Concat(new object[] { "Received RPC \"", str, "\" on viewID ", viewID, " with a prefix of ", num2, ", our prefix is ", photonView.prefix, ". The RPC has been ignored." + playerid(sender) }));
                }
                else if (str == string.Empty)
                {
                    Debug.LogError("Malformed RPC; this should never occur. Content: " + SupportClass.DictionaryToString(rpcData) + "" + playerid(sender));
                }
                else
                {
                    if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
                    {
                        Debug.Log("Received RPC: " + str + "" + playerid(sender));
                    }
                    if ((photonView.group == 0) || this.allowedReceivingGroups.Contains(photonView.group))
                    {
                        ...
                        foreach (MonoBehaviour behaviour in photonView.GetComponents<MonoBehaviour>())
                        {
                            if (behaviour == null)
                            {
                                Debug.LogError("ERROR You have missing MonoBehaviours on your gameobjects!" + playerid(sender));
                            }
                            else
                            {
                        ...
                        if (num7 != 1)
                        {
                            ...
                            if (num7 == 0)
                            {
                                if (num8 == 0)
                                {
                                    Debug.LogError(string.Concat(new object[] { "PhotonView with ID ", viewID, " has no method \"", str, "\" marked with the [RPC](C#) or @RPC(JS) property! Args: ", str2  + playerid(sender) }));
                                }
                                else
                                {
                                    Debug.LogError(string.Concat(new object[] { "PhotonView with ID ", viewID, " has no method \"", str, "\" that takes ", callParameterTypes.Length, " argument(s): ", str2 + playerid(sender) }));
                                }
                            }
                            else
                            {
                                Debug.LogError(string.Concat(new object[] { "PhotonView with ID ", viewID, " has ", num7, " methods \"", str, "\" that takes ", callParameterTypes.Length, " argument(s): ", str2, ". Should be just one?" + playerid(sender) }));
                            }
                        }
                    }
                }
            ...
            {
                Debug.LogError("Malformed RPC; this should never occur. Content: " + SupportClass.DictionaryToString(rpcData) + " PlayerID:" + sender.ID);
        ...
        {
            Debug.LogError("RPCDATA is null."+ playerid(sender) );
        }
        else if (rpcData.Count < 0)
        {
            Debug.LogError("RPCDATA is clean." + playerid(sender));
        }
    ...
    bool check_custom_prop(ExitGames.Client.Photon.Hashtable hash, int id_toplayer, PhotonPlayer in_player)
    {
        if (PhotonNetwork.isMasterClient && hash["isTitan"] != null && id_toplayer != in_player.ID&& cext.returnIntFromObject(hash["isTitan"]) == 2)
        {
            FengGameManagerMKII.instance.kickPlayerRC(in_player, false, INC.la("dis_property_player_is_tit"));
        }
        if (id_toplayer == PhotonNetwork.player.ID)
        {
            if (hash["name"] != null)
            {
            ...
            if ((in_player.isMasterClient || in_player.isLocal) || hash["kills"] != null)
            {
                return true;
            }
            PhotonNetwork.player.SetCustomProperties(PhotonNetwork.player.customProperties);
            Debug.LogError("Chagu you custom property. ID:" + in_player.ID + " hastable: " + hash.ToStringFull());
            return false;
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        if (sender != null && sender == PhotonNetwork.player && Application.loadedLevel != 0)
        {
            UnityEngine.Debug.LogError("Cyan_mod Error Playred ");
            PanelInformer.instance.Add("Cyan_mod Error Playred ", PanelInformer.LOG_TYPE.WARNING);
            return;
        }
        if (sender != null && PhotonNetwork.networkingPeer.TrafficStatsEnabled)
        {
            if (sender.bytes > 999999999)
            {
                sender.bytes = 0;
            }
            sender.bytes = sender.bytes + (PhotonNetwork.networkingPeer.ByteCountCurrentDispatch);
        ...
        switch (photonEvent.Code)
        {
            ...
            case 203:
                ...
                {
                    FengGameManagerMKII.instance.gameObject.AddComponent<OnLeave>();
                }
                goto Label_0D1F;
            ...
            case 204:
                ...
                if (photonEvent[245] is ExitGames.Client.Photon.Hashtable)
                {
                    ...
                    if (hashtable3[(byte)0] is int)
                    {
                        ...
                        if (FengGameManagerMKII.instance.photonView.viewID != num8)
                        {
                        ...
                        {
                            PanelInformer.instance.Add("Removed_Multypleer ID" + sender.ID, PanelInformer.LOG_TYPE.DANGER);
                            UnityEngine.Debug.LogError("Removed_Multypleer ID" + sender.ID);
                        }
                    }
                }
                goto Label_0D1F;
            ...
            case 228:
                if (!((sender != null) && !FengGameManagerMKII.ignoreList.Contains(sender.ID)) && (!sender.isMasterClient || !sender.isLocal))
                {
                    return;
                }
            ...
            case 253:
              if (sender == null || !FengGameManagerMKII.ignoreList.Contains(sender.ID))
				{
					object ghh444 = photonEvent[253];
                    if (ghh444 is int)
					{
                        int num1115 = (int)ghh444;
						ExitGames.Client.Photon.Hashtable hashtable10 = null;
						ExitGames.Client.Photon.Hashtable hashtable11 = null;
						if (num1115 == 0)
						{
							object gh8888 = photonEvent[251];
                            if (gh8888 == null || !(gh8888 is ExitGames.Client.Photon.Hashtable))
							{
								return;
							}
                            hashtable10 = (ExitGames.Client.Photon.Hashtable)gh8888;
						}
						else
						{
                            object gh8888 = photonEvent[251];
                            if (gh8888 is ExitGames.Client.Photon.Hashtable)
							{
                                hashtable11 = (ExitGames.Client.Photon.Hashtable)gh8888;
								if (sender != null)
								{
									hashtable11["sender"] = sender;
									if (PhotonNetwork.isMasterClient && !sender.isLocal && num1115 == sender.ID && (hashtable11.ContainsKey("statACL") || hashtable11.ContainsKey("statBLA") || hashtable11.ContainsKey("statGAS") || hashtable11.ContainsKey("statSPD")))
									{
										if (hashtable11.ContainsKey("statACL") && (int)(hashtable11["statACL"]) > 150)
										{
											FengGameManagerMKII.instance.kickPlayerRC(sender, true, "excessive stats.");
											return;
										}
                                        if (hashtable11.ContainsKey("statBLA") && (int)(hashtable11["statBLA"]) > 125)
										{
											FengGameManagerMKII.instance.kickPlayerRC(sender, true, "excessive stats.");
											return;
										}
                                        if (hashtable11.ContainsKey("statGAS") && (int)(hashtable11["statGAS"]) > 150)
										{
											FengGameManagerMKII.instance.kickPlayerRC(sender, true, "excessive stats.");
											return;
										}
                                        if (hashtable11.ContainsKey("statSPD") && (int)(hashtable11["statSPD"]) > 140)
										{
											FengGameManagerMKII.instance.kickPlayerRC(sender, true, "excessive stats.");
											return;
										}
									}
									if ((int)FengGameManagerMKII.settings[393] == 1 && hashtable11.ContainsKey("name"))
									{
										if (num1115 != sender.ID)
										{
											InstantiateTracker.instance.resetPropertyTracking(num1115);
										}
                        ...
                        if (check_custom_prop( hashtable11, num1115, sender))
                        {
                            this.ReadoutProperties(hashtable10, hashtable11, num1115);
                        }
                        goto Label_0D1F;
					}
                    goto Label_0D1F;
            ...
            case 255:
                ...
                mLocalActor.CM = true;
                mLocalActor.version = UIMainReferences.CyanModVers;
                goto Label_0D1F;
            case 101:
                {
                    PhotonPlayer player2 = sender;
                    ExitGames.Client.Photon.Hashtable hashtable7 = photonEvent[245] as ExitGames.Client.Photon.Hashtable;
                    if (hashtable7 != null)
                    {
                        if (sender == null)
                        {
                            if ((key > 0) && this.mActors.ContainsKey(key))
                            {
                                player2 = this.mActors[key];
                            }
                            else if (hashtable7.ContainsKey(102) && (hashtable7[102] is short))
                            {
                                short num1111 = (short)hashtable7[102];
                                if (!this.mActors.TryGetValue(num1111, out player2))
                                {
                                    player2 = PhotonPlayer.Find(num1111);
                                }
                            }
                        }
                        if (player2 != null)
                        {
                            if (hashtable7[104] is string)
                            {
                                player2.version = (string)hashtable7[104];
                            }
                            if (hashtable7[103] is string)
                            {
                                player2.versionRS = (string)hashtable7[103];
                            }
                            if (hashtable7[102] is bool)
                            {
                                player2.CM = (bool)hashtable7[102];
                            }
                            if (hashtable7[101] is bool)
                            {
                                player2.RS = (bool)hashtable7[101];
                            }
                            UnityEngine.Debug.Log(hashtable7.ToStringFull() + "\n" + player2.ID.ToString());
                        }
                        this.RebuildPlayerListCopies();
                    }
                    return;
    ...
    public void RemoveInstantiatedGO(GameObject go, bool localOnly)
    {
        ...
        else if (go != FengGameManagerMKII.instance.gameObject)
        {
    ...
    public void AddLocalRPC(string rpc)
    {
        if (isInformationEvent && rpc != null)
        {
            if (mLocalActor.eve == null)
            {
                mLocalActor.eve = new Eve();
            }
            mLocalActor.eve.addrpc(rpc);
        }
    }
    ...
    public static void SendMonoMessage(PhotonNetworkingMessage methodString, params object[] parameters)
    {
        ...
        {
            FengGameManagerMKII.instance.gameObject.SendMessage(methodName, parameters[0], SendMessageOptions.DontRequireReceiver);
        }
        else
        {
            FengGameManagerMKII.instance.gameObject.SendMessage(methodName, parameters, SendMessageOptions.DontRequireReceiver);
        }
    }
