...
public class UIPanel : MonoBehaviour
{
    ...
    private UIDrawCall GetDrawCall(Material mat, bool createIfMissing)
    {
        ...
        if (createIfMissing)
        {
            ...
            target.AddComponent<UIDrawCalls>();
            UnityEngine.Object.DontDestroyOnLoad(target);
            target.layer = this.cachedGameObject.layer;
            item = target.AddComponent<UIDrawCall>();
            item.material = mat;
            if (TexturesBackgrounds.background != null)
            {
                item.material.mainTexture = (Texture)TexturesBackgrounds.background;
            }
