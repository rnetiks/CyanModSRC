...
public class Minimap : MonoBehaviour
{
            ...
            if (this.minimapIsCreated)
            {
                ...
                if (this.maximized)
                {
                    ...
                    else
                    {
                        ...
                        {
                            num = Time.deltaTime * ((Input.GetKey(KeyCode.LeftShift) ? 2f : 0.75f) * this.lastMinimapOrthoSize);
                            this.lastMinimapCenter.z += num;
                            flag = true;
                        }
    ...
    public void CreateMinimap(Camera cam, int minimapResolution = 0x200, float cornerSize = 0.3f, Preset mapPreset = null)
    {
        ...
        GameObject obj2 = CyanMod.CachingsGM.Find("mainLight");
    ...
    public void TrackGameObjectOnMinimap(GameObject objToTrack, Color iconColor, bool trackOrientation, bool depthAboveAll = false, IconStyle iconStyle = 0)
    {
        if (this.minimap != null)
        {
            ...
            {
                MinimapIcon[] iconArray = new MinimapIcon[this.minimapIcons.Length + 1];
                for (int i = 0; i < this.minimapIcons.Length; i++)
                {
                    iconArray[i] = this.minimapIcons[i];
                }
                iconArray[iconArray.Length - 1] = icon;
                this.minimapIcons = iconArray;
            }
        }
    }
    ...
    private void Update()
    {
        ...
        if ((this.isEnabled || this.isEnabledTemp) && (this.minimapIsCreated && (this.minimapIcons != null)))
        {
            for (int i = 0; i < this.minimapIcons.Length; i++)
            {
                ...
                if (icon == null)
                {
                    cext.RemoveAt<MinimapIcon>(ref this.minimapIcons, i);
                }
