...
public static class PhotonNetwork
{
    ...
    private static int AllocateViewID(int ownerId)
    {
        if (ownerId != 0)
        {
            ...
            for (int j = 1; j < MAX_VIEW_IDS; j++)
            {
                ...
                {
                    int key = lastUsedViewSubId + num2;
            ...
            throw new Exception(
                String.Format("AllocateViewID() failed. User {0} is out of subIds, as all viewIDs are used.", ownerId));
        }
        ...
        for (int i = 1; i < MAX_VIEW_IDS; i++)
        {
            ...
            {
                int num8 = lastUsedViewSubIdStatic + num6;
                if (!networkingPeer.photonViewList.ContainsKey(num8))
                {
        ...
        throw new Exception(
            String.Format("AllocateViewID() failed. Room (user {0}) is out of subIds, as all room viewIDs are used.",
                ownerId));
    }
    ...
    public static void DestroyPlayerObjects(PhotonPlayer targetPlayer, bool local = false)
    {
        if (targetPlayer == null)
        {
            Debug.LogError("DestroyPlayerObjects() failed, cause parameter 'targetPlayer' was null.");
        }
        if ((targetPlayer != player) && (targetPlayer.ID != player.ID))
        {
            DestroyPlayerObjects(targetPlayer.ID, local);
        }
    }
    ...
    public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, int group, object[] data)
    {
        ...
        GameObject resourceGameObject = CyanMod.CacheGameResources.Load<GameObject>(prefabName);
        ...
        if (resourceGameObject.GetComponent<PhotonView>() == null)
        {
            UnityEngine.Debug.LogError("Failed to InstantiateSceneObject prefab:" + prefabName + ". Prefab must have a PhotonView component.");
            return null;
        }
        int[] viewIDs = AllocateSceneViewIDs(resourceGameObject.GetComponentsInChildren<PhotonView>(true).Length);
        if (viewIDs == null)
        {
            UnityEngine.Debug.LogError(string.Concat(new object[] { "Failed to InstantiateSceneObject prefab: ", prefabName, ". No ViewIDs are free to use. Max is: ", (short)0x3e8 }));
            return null;
        }
        return networkingPeer.DoInstantiate(networkingPeer.SendInstantiate(prefabName, position, rotation, group, viewIDs, data, true), networkingPeer.mLocalActor, resourceGameObject);
    }
    ...
    public static void UnAllocateViewID(int viewID)
    {
        ...
        {
            Debug.LogWarning(
                String.Format("Unallocated manually used viewID: {0} but found it used still in a PhotonView: {1}",
                    viewID, networkingPeer.photonViewList[viewID]));
        }
    }
    ...
    {
        get { return (connectionStatesDetailed == PeerStates.Joined); }
    }
    ...
    {
        get { return (0x3e8 / sendIntervalOnSerialize); }
        set
        {
            if (value > sendRate)
            {
    ...
    {
        get { return networkingPeer.LimitOfUnreliableCommands; }
        set
        {
