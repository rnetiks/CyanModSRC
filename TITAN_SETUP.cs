    ...
    public void setHair2()
    {
        ...
        if ((((int) FengGameManagerMKII.settings[1]) == 1) && ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine))
        {
            ...
            if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && base.photonView.isMine)
            {
                if (flag)
                {
                    objArray = new object[] { num, eye, hairlink };
                    base.photonView.RPC("setHairRPC2", PhotonTargets.AllBuffered, objArray);
                }
                else
                {
                    color = HeroCostume.costume[UnityEngine.Random.Range(0, HeroCostume.costume.Length - 5)].hair_color;
                    objArray = new object[] { num, eye, color.r, color.g, color.b };
                    base.photonView.RPC("setHairPRC", PhotonTargets.AllBuffered, objArray);
                }
            }
        ...
        else
        {
            ...
            if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && base.photonView.isMine)
            {
                objArray = new object[] { this.hairType, id, this.part_hair.renderer.material.color.r, this.part_hair.renderer.material.color.g, this.part_hair.renderer.material.color.b };
                base.photonView.RPC("setHairPRC", PhotonTargets.OthersBuffered, objArray);
            }
        }
    }
