...
public class SimpleAES
{
    ...
    public byte[] this[byte[] buffer, bool encrypt]
    {
        get
        {
            if (encrypt)
    ...
    public string this[string buffer, bool encrypt]
    {
        get
        {
            if (encrypt)
            {
                return Convert.ToBase64String(this.Encrypt(this.encoder.GetBytes(buffer)));
            }
            return this.encoder.GetString(this.Decrypt(Convert.FromBase64String(buffer)));
        }
    }

    public string this[string encrypted]
    {
        get
        {
            return this.encoder.GetString(this.Decrypt(Convert.FromBase64String(encrypted)));
